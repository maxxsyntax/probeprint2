...
./standalone_seqgraph.sh --pnl >/dev/null 2>&1
...
report=$(./standalone_seqgraph.sh --validate 2>&1)
...
SEQGRAPH_GATE_IE=0 ./standalone_seqgraph.sh --recompute >/dev/null 2>&1
...
assert_eq "and one real device is scattered across clusters" "1" \
    "$(sq1 "select count(*) > 0 from (
              select wlan_sa from ssid
               where wlan_sa in ('00:11:22:00:00:01','00:33:44:00:00:01')
               group by wlan_sa having count(distinct device_id) > 1) x;")"

ungated=$(SEQGRAPH_GATE_IE=0 ./standalone_seqgraph.sh --validate 2>&1)
assert_contains "the harness detects the false merge" \
    "FALSE MERGES (>1 static MAC per cluster)   : 1" "$ungated"
assert_contains "the harness detects the false split too" \
    "FALSE SPLITS (1 static MAC over >1 cluster): 1" "$ungated"
